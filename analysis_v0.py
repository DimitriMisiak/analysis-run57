...
def go_analysis(run_name):
    ...
    def ion_chi2_cut(num, chi2_thresh=300):
        """ Return the indexes of the events passing the chi2 cut on the heat
        channel for the given partition.  
        Also return the truth array.
        """
    ...
    volt_config = np.where(volt_config==0, 1, volt_config)
#    print(volt_config)
...
#    energy_ion_bd = energy_cut_ion[1] + energy_cut_ion[3] + energy_cut_ion[0] + energy_cut_ion[2]
...
#    energy_ion_bd_fid = energy_fid_ion[1] + energy_fid_ion[3] + energy_fid_ion[0] + energy_fid_ion[2]    
    ### Histogramm check
    ...
    # calib coeff ADU/keV
    calib_chal = sens/ ( (10.37) * (1 + v_bias / 3) )
...
#                'tg18l005',
    ...
    import tqdm
    for run_name in tqdm.tqdm(run_list):
        go_analysis(run_name)
#        plt.close('all')
